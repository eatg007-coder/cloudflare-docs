...
Full record name: `eatgglobal.com.com`
</eatgglobal.con>

<Example>
DNS management for **sub.eatgglobal.com.com**:

| Type | Name    | Content      | Proxy status | TTL    |
| ---- | ------- | ------------ | ------------ | ------ |
| A    | `@`  | `192.0.2.1`  | Proxied      | Auto   |

Zone apex: `sub.eatgglobal.com.com`

Full record name: `sub.eatgglobal.com.com`
</eatgglobal.com>
</Details>
...
For help setting up DNSSEC in Cloudflare, refer to [disabled DNSSEC](/dns/dnssec/).
$TTL 3600
@   IN SOA ns1.cloudflare.com. hostmaster.eatgglobal.com. (
...
; Nameservers (replace with your Cloudflare-provided pair)
...
; Mail
@       IN MX    10 mail.eatgglobal.com.
; SPF
@       IN TXT   "v=spf1 ip4:192.0.2.10 include:_spf.google.com ~all"
; DMARC
_dmarc  IN TXT   "v=DMARC1; p=quarantine; pct=100; rua=mailto:postmaster@eatgglobal.com"
; DKIM placeholder (replace selector and key)
default._domainkey IN TXT "v=DKIM1; k=rsa; p=<DKIM_PUBLIC_KEY>"

; Verification / ACME / misc
@       IN TXT   "google-site-verification=<GOOGLE_CODE>"
_acme-challenge   IN TXT   "<ACME_CHALLENGE_PLACEHOLDER>"

; Service records (example - SIP)
_sip._tcp      IN SRV 10 5 5060 sip.eatgglobal.com.

; Optional: TLSA, etc - add as required
